...
"""
=============================================================================
Repo Code Empathizer v2.2.2 - Aplicación Principal (CLI)
=============================================================================

Este módulo es el punto de entrada principal de Code Empathizer, una herramienta
de análisis estático avanzada para medir la alineación técnica entre el código
de una empresa (repositorio maestro) y el código de candidatos.

DESCRIPCIÓN GENERAL:
-------------------
Code Empathizer evalúa múltiples métricas de calidad de código para determinar
qué tan compatible es el estilo de programación de un candidato con el de una
empresa. Esto facilita procesos de contratación técnica y onboarding.

CARACTERÍSTICAS PRINCIPALES:
---------------------------
- Análisis multi-lenguaje: Python, JavaScript, TypeScript, Java, Go, C#, C++,
  PHP, Ruby, Swift, HTML, CSS
- 11 categorías de análisis: nombres, documentación, modularidad, complejidad,
  manejo de errores, pruebas, seguridad, consistencia, patrones, rendimiento,
  comentarios
- Múltiples formatos de exportación: TXT, JSON, HTML, Dashboard interactivo, PNG
- Modo equipo para comparar múltiples candidatos simultáneamente
- Sistema de caché para optimizar análisis repetidos
- Análisis local mediante clonación de repositorios

MODOS DE EJECUCIÓN:
------------------
1. Interactivo: python3 src/main.py
2. CLI básico:  python3 src/main.py --empresa user/repo --candidato user/repo
3. Modo equipo: python3 src/main.py --empresa user/repo --candidatos a b c --team-mode

EJEMPLOS DE USO:
---------------
    # Análisis básico interactivo
    $ python3 src/main.py

    # Análisis con argumentos
    $ python3 src/main.py --empresa django/django --candidato pallets/flask --output html

    # Modo equipo con múltiples candidatos
    $ python3 src/main.py --empresa facebook/react --candidatos vuejs/vue angular/angular --team-mode --output all

DEPENDENCIAS:
------------
- PyGithub: Acceso a la API de GitHub
...
-------------------------------
- GITHUB_TOKEN: Token de acceso personal de GitHub con permisos public_repo
...
import argparse    # Parsing de argumentos de línea de comandos
import logging     # Sistema de logging para depuración
from datetime import datetime  # Manejo de fechas y timestamps
from typing import List       # Type hints para mejor documentación

# Librerías de terceros
from dotenv import load_dotenv  # Carga de variables desde archivo .env
import yaml                      # Parser de archivos YAML de configuración

# Módulos internos de Code Empathizer
from github_utils import GitHubRepo                    # Cliente de API de GitHub
from language_analyzers.factory import AnalyzerFactory # Factory de analizadores
from exporters import Exporter                         # Generador de reportes
from parallel_analyzer import ParallelAnalyzer         # Análisis en paralelo
from cache_manager import CacheManager, CachedAnalyzer # Sistema de caché
from empathy_algorithm import EmpathyAlgorithm         # Motor de cálculo de empatía

# =============================================================================
# CONFIGURACIÓN GLOBAL
# =============================================================================

# Configurar el sistema de logging con formato detallado
# Nivel INFO para mostrar progreso, DEBUG para depuración completa
logging.basicConfig(
    level=logging.INFO,
...
# Esto incluye GITHUB_TOKEN necesario para la API de GitHub
load_dotenv()

# =============================================================================
# CONSTANTES DE FORMATO (Colores ANSI para terminal)
# =============================================================================
# Códigos de escape ANSI para formateo de texto en terminal
# Compatible con la mayoría de terminales Unix/Linux/macOS y Windows Terminal
COLORS = {
    'header': '\033[95m',      # Magenta - encabezados principales
    'blue': '\033[94m',        # Azul - información general
    'cyan': '\033[96m',        # Cyan - información secundaria
    'green': '\033[92m',       # Verde - éxito/completado
    'success': '\033[92m',     # Alias para verde (éxito)
    'warning': '\033[93m',     # Amarillo - advertencias
    'fail': '\033[91m',        # Rojo - errores
    'end': '\033[0m',          # Reset - volver al color por defecto
    'bold': '\033[1m',         # Negrita
...
def get_output_format() -> str:
    ...
    print("  5. Gráficas ZIP - Todas las gráficas en un archivo ZIP")
    print("  6. Todos - Generar todos los formatos + ZIP")
    ...
    while True:
        try:
            ...
            if opcion in formatos:
                ...
                print(f"{COLORS['warning']}Por favor seleccione una opción válida (1-6){COLORS['end']}")
                
        except KeyboardInterrupt:
            print(f"\n{COLORS['warning']}Operación cancelada{COLORS['end']}")
            exit(0)
        except Exception:
            print(f"{COLORS['warning']}Por favor ingrese un número válido{COLORS['end']}")
        ...
        try:
            if repos_disponibles:
                ...
                if 1 <= opcion_num <= len(repos_disponibles):
                    ...
                    print(f"{COLORS['warning']}Opción inválida{COLORS['end']}")
                    continue
            else:
                repo_input = input("Ingrese URL de GitHub o formato 'usuario/repo': ")
                repo_seleccionado = GitHubRepo.extraer_usuario_repo(repo_input)
            ...
            if repo_previo and repo_seleccionado == repo_previo:
                print(f"{COLORS['fail']}ERROR:Error: El repositorio del candidato no puede ser el mismo que el de la empresa{COLORS['end']}")
            ...
            print(f"{COLORS['fail']}ERROR:Error: {str(e)}{COLORS['end']}")
...
def analyze_team_mode(github_repo: GitHubRepo, repo_empresa: str, candidatos: List[str], 
    ...
    if not analisis_empresa:
        print(f"{COLORS['fail']}ERROR:Error analizando repositorio de empresa{COLORS['end']}")
        return
    ...
    for i, candidato in enumerate(candidatos, 1):
        ...
        try:
            ...
            if candidato_repo == repo_empresa:
                print(f"{COLORS['warning']}Saltando: El candidato no puede ser el mismo que la empresa{COLORS['end']}")
                continue
            ...
            if analisis_candidato:
                ...
                print(f"{COLORS['fail']}ERROR:Error analizando candidato: {candidato}{COLORS['end']}")
                
        except Exception as e:
            print(f"{COLORS['fail']}ERROR:Error con candidato {candidato}: {str(e)}{COLORS['end']}")
    ...
    if resultados_equipo['candidatos']:
        ...
        print(f"{COLORS['fail']}ERROR:No se pudo analizar ningún candidato{COLORS['end']}")
...
def main() -> None:
    ...
    parser.add_argument('--output', type=str, default=None,
                       choices=['txt', 'json', 'html', 'dashboard', 'graficas_zip', 'all'],
    ...
    try:
        ...
        if args.team_mode or args.candidatos:
            ...
            if args.candidatos:
                ...
                if not candidatos:
                    print(f"{COLORS['fail']}ERROR:Debe ingresar al menos un candidato{COLORS['end']}")
                    return
        ...
        if not resultados['repos']['empresa'] or not resultados['repos']['candidato']:
            print(f"{COLORS['fail']}ERROR:Error en el análisis de uno o ambos repositorios{COLORS['end']}")
            return
        ...
        if output_format in ['graficas_zip', 'all']:
            exporter.exportar_graficas_zip(resultados, timestamp)
            print(f"{COLORS['green']}Gráficas exportadas a ZIP{COLORS['end']}")
        ...
        print(f"\n{COLORS['warning']}Análisis cancelado por el usuario{COLORS['end']}")
    except Exception as e:
        print(f"\n{COLORS['fail']}ERROR:Error: {str(e)}{COLORS['end']}")
...
def mostrar_resumen(resultados: dict) -> None:
    ...
    if 'empathy_analysis' in resultados:
        ...
        if lang_overlap['missing']:
            print(f"  Lenguajes faltantes: {', '.join(lang_overlap['missing'])}")
