...
"""
=============================================================================
Módulo de Algoritmo de Empatía - Motor de Cálculo de Compatibilidad
=============================================================================

Este módulo implementa el algoritmo central de Code Empathizer para calcular
la puntuación de empatía entre el código de una empresa y un candidato.

FÓRMULA MATEMÁTICA:
------------------
La puntuación de empatía se calcula mediante la siguiente fórmula:

    E = (Σ Si × Wi × Ci) × L × Π Fk

Donde:
    - E  = Puntuación de empatía final (0-100)
    - Si = Puntuación de categoría i (nombres, documentación, etc.)
    - Wi = Peso de categoría i (definido en category_weights)
    - Ci = Factor de correlación entre categorías
    - L  = Factor de coincidencia de lenguajes
    - Fk = Factores de ajuste múltiples (6 factores)

CATEGORÍAS DE ANÁLISIS (11 total):
---------------------------------
Categorías base (70% del peso):
    1. nombres (12%)           - Descriptividad de variables y funciones
    2. documentacion (12%)     - Cobertura de docstrings y comentarios
    3. modularidad (10%)       - Organización en módulos y componentes
    4. complejidad (10%)       - Complejidad ciclomática y anidación
    5. manejo_errores (8%)     - Cobertura de excepciones
    6. pruebas (8%)            - Detección de tests y aserciones
    7. seguridad (6%)          - Validación de entradas
    8. consistencia_estilo (4%) - Formato y espaciado

Categorías avanzadas (30% del peso):
    9. patrones (12%)          - Patrones de diseño y anti-patrones
    10. rendimiento (10%)      - Operaciones costosas y complejidad
    11. comentarios (8%)       - Ratio código/comentarios y TODOs

FACTORES DE AJUSTE:
------------------
    F1: Factor de complejidad (penaliza diferencias extremas entre repos)
    F2: Factor de consistencia (bonifica homogeneidad en métricas)
    F3: Factor de excelencia (bonifica >85% en categorías críticas)
    F4: Factor de anti-patrones (penaliza god class, spaghetti code)
    F5: Factor de patrones (bonifica uso de design patterns)
    F6: Factor de balance (bonifica distribución equilibrada)

INTERPRETACIÓN DE RESULTADOS:
----------------------------
    90-100%: Excelente - Candidato altamente recomendado
    75-89%:  Bueno - Recomendado con capacitación menor
    60-74%:  Aceptable - Viable con plan de capacitación
    45-59%:  Bajo - Requiere capacitación significativa
    0-44%:   Muy Bajo - No recomendado sin capacitación extensiva

EJEMPLO DE USO:
--------------
    >>> from empathy_algorithm import EmpathyAlgorithm
    >>> algorithm = EmpathyAlgorithm()
    >>> result = algorithm.calculate_empathy_score(empresa_metrics, candidato_metrics)
    >>> print(f"Puntuación: {result['empathy_score']}%")
...
class EmpathyAlgorithm:
    ...
    def _cosine_similarity(self, a: float, b: float) -> float:
        ...
        # Optimizado: para escalares, sqrt(a²) = |a|
        dot_product = a * b
