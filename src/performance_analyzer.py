...
Este módulo detecta problemas de rendimiento potenciales en el código fuente
mediante análisis estático de patrones conocidos que causan degradación.

PROBLEMAS DE RENDIMIENTO DETECTADOS:
-----------------------------------
CRÍTICOS (severity: critical):
    - database_in_loop: Queries de BD dentro de bucles -> O(n) llamadas
    - triple_nested_loops: Bucles triplemente anidados -> O(n³)

ALTOS (severity: high):
    - nested_loops: Bucles doblemente anidados -> O(n²)

MEDIOS (severity: medium):
    - inefficient_string_concat: Concatenación de strings en loops
    - inefficient_list_operations: insert(0), remove() en loops
    - recursive_calls: Llamadas recursivas sin memoización

BAJOS (severity: low):
    - synchronous_io: Operaciones I/O bloqueantes

OPTIMIZACIONES DETECTADAS:
-------------------------
Patrones positivos que indican buenas prácticas:
    - caching: @cache, @memoize, @lru_cache
    - async_operations: async/await, Promise
    - bulk_operations: executemany, bulk_create
    - efficient_data_structures: set(), dict(), deque()

ANÁLISIS DE COMPLEJIDAD:
-----------------------
Calcula la complejidad ciclomática de funciones:
    - Cuenta estructuras de control (if, for, while, try, case)
    - Identifica funciones con complejidad > 10 (alta)

MÉTRICAS CALCULADAS:
-------------------
- performance_issues: Problemas por tipo con ubicación
- optimizations_found: Optimizaciones detectadas
- complexity_analysis: Funciones con alta complejidad
- performance_score: Puntuación de rendimiento (0-100)
...
--------------
    ...
    >>> files = {"main.py": "for x in items:\n  db.query(x)"}
    >>> results = analyzer.analyze_performance(files)
    >>>
    >>> print(f"Problemas críticos: {len(results['performance_issues'])}")
    >>> print(f"Score: {results['performance_score']}")

INTERPRETACIÓN DEL SCORE:
------------------------
- >= 80: Buen rendimiento general
- 60-79: Rendimiento aceptable, áreas de mejora
- < 60: Se recomienda revisar y optimizar
...
# IMPORTACIONES
# =============================================================================
import re                            # Expresiones regulares para detección
...
class PerformanceAnalyzer:
    ...
    def __init__(self):
        ...
        self.expensive_patterns = {
            ...
            'recursive_calls': {
                'patterns': [
                    # Simplificado - solo detectar llamadas recursivas básicas
                    # Nota: estos patrones son más simples para evitar backtracking
                ],
                'severity': 'medium',
                'complexity': 'Varies'
            },
    ...
    def analyze_performance(self, files: Dict[str, str]) -> Dict[str, Any]:
        ...
        print(f"            Analizando rendimiento de {len(files)} archivos...", flush=True)

        # Analizar cada archivo
        for idx, (file_path, content) in enumerate(files.items(), 1):
            if idx % 10 == 0:
                print(f"            Procesado {idx}/{len(files)} archivos...", flush=True)
    ...
    def _detect_performance_issues(self, content: str, file_path: str) -> Dict[str, List[Dict]]:
        ...
        # Limitar tamaño de contenido para evitar regex lento
        if len(content) > 100000:  # Si el archivo es muy grande (>100KB)
            # Solo analizar primeras 100K para evitar regex extremadamente lento
        ...
        for issue_type, issue_def in self.expensive_patterns.items():
            ...
            for pattern in patterns:
                try:
                    # Usar timeout implícito limitando el contenido
                    # Remover DOTALL que causa catastrophic backtracking
                    matches = re.finditer(pattern, content, re.MULTILINE)
                    match_count = 0
                    for match in matches:
                        match_count += 1
                        if match_count > 50:  # Limitar a 50 matches por patrón
                            break
