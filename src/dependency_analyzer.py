...
Este módulo analiza las dependencias e imports en proyectos de código,
identificando relaciones entre archivos, dependencias externas e internas,
y detectando posibles dependencias circulares.
...
- PHP: use, require, require_once, include
- Ruby: require, require_relative, load

MÉTRICAS CALCULADAS:
-------------------
- total_dependencies: Total de dependencias únicas
- external_dependencies: Dependencias externas (librerías)
- internal_dependencies: Dependencias internas (proyecto)
- dependency_ratio: Ratio de archivos con dependencias
- avg_dependencies_per_file: Promedio de dependencias por archivo
- circular_dependencies: Número de ciclos detectados
- coupling_score: Score de acoplamiento (0-1)

DETECCIÓN DE DEPENDENCIAS CIRCULARES:
------------------------------------
El módulo implementa un algoritmo de detección de ciclos basado en DFS
(Depth-First Search) para identificar dependencias circulares que pueden
causar problemas de importación o indicar diseño problemático.

CLASIFICACIÓN DE DEPENDENCIAS:
-----------------------------
- Internas: Archivos del mismo proyecto (comienzan con . o /)
- Externas: Librerías y paquetes de terceros
...
--------------
    ...
    >>> results = analyzer.analyze_dependencies(files)
    >>>
    >>> print(f"Total dependencias: {results['total_dependencies']}")
    >>> print(f"Ciclos detectados: {results['circular_dependencies']}")
...
# IMPORTACIONES
# =============================================================================
import re                            # Expresiones regulares para patrones de import
from typing import Dict, List, Set, Any, Optional  # Type hints
